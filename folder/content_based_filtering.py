...
def get_lat_lon_from_address(address, access_token):
    """Get latitude and longitude of an address using LocationIQ API."""
    try:
...
def filter_by_location(user_lat, user_lon, restaurants, radius=7):
    """Filter restaurants within a certain radius of the user's location."""
...
data = load_csv(r"C:\Users\Nithin\OneDrive\Desktop\new\preprocessed_data.csv")
data['profile'] = (
    data['Cuisine'].fillna('') + " " +
